...
#pivot_df = df.pivot(index='action_date_fiscal_year', columns='agency_abb', values='awardtotal')

pivot_df.to_csv ('AllCumulativeCounts.csv', header=True)
#pivot_df.to_csv ('FilteredCumulativeCounts.csv', header=True)
#pivot_df.to_csv ('AwardTotal.csv', header=True)

matplotlib.style.use('ggplot')
pivot_df.loc[:,['DHS', 'DOC', 'DOD', 'DOJ', 'GSA', 'HHS', 'SEC', 'TREAS', 'USDA', 'VA']] \
.plot.bar(stacked=True, figsize=(12,7), edgecolor='black',legend=False, color=colorWheel)

plt.xlabel('Fscal Year')
plt.ylabel('All Award Count - Cumulative')
