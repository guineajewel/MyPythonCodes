...
try:
    for m in range(len(agency_abb)):
        df1 = pd.read_sql_query(sqst.sqlst25.format(agency_abb[m]), engine)
        df1.to_csv (agency_abb[m]+'_Top15Vendors.csv', header=True)

        table_name = agency_abb[m]+"_Top15Vendors_detail"
        print(table_name)    
        #x=0
        for x in range(15):
            df2 = pd.read_sql_query(sqst.sqlst26.format(agency_abb[m],df1['recipient_duns'][x]), engine)
            df2['rank'] = x #+ 1    
            print('rank written..')
            if x == 0:
