...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 25 18:10:03 2020
...
used allagency2 table from the database and SQL37 statement from the sqlstmnts.py file
prvious version analysis2020Nov08-1.py used allagency1 table and SQL17 
...
for m in range(len(agency_abb)):
    ...
    sqlst = sqst.sqlst37.format(agency_abb[m])
    ...
    if m == 9:
        ax2.figure.legend(['Unfiltered Award','Filtered Award'], bbox_to_anchor=(0.20, -0.15, 0.5, 0.5))        
    if m in (1,3,5,6,8):
    ...
    time.sleep(1)
    plt.savefig('figs/'+agency_abb[m]+'awardcount2009to2019-2.png', dpi=300)
    plt.show()
    ...
    df.to_csv ('rda/'+agency_abb[m]+'awardcount2009to2019-2.csv', header=True)